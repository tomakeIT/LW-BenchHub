...
def approach_ee_handle(env: ManagerBasedRLEnv, threshold: float, target_frame: str) -> torch.Tensor:
    ...
    handle_pos = env.scene[target_frame].data.target_pos_w[..., 0, :]
    ...
    reward = -torch.log(distance**2 + 1)
    # return torch.where(distance <= threshold, 2 * reward, reward)
    return reward


def align_ee_handle(env: ManagerBasedRLEnv, target_frame: str) -> torch.Tensor:
    ...
    handle_quat = env.scene[target_frame].data.target_quat_w[..., 0, :]
    ...
    align_z = torch.bmm(ee_tcp_x.unsqueeze(1), handle_z.unsqueeze(-1)).squeeze(-1).squeeze(-1)
...
def align_grasp_around_handle(env: ManagerBasedRLEnv, target_frame: str) -> torch.Tensor:
    ...
    thumb_pos = ee_fingertips_w[..., 0, :]
    index_pos = ee_fingertips_w[..., 1, :]
    middle_pos = ee_fingertips_w[..., 2, :]

    # Check if hand is in a graspable pose
    is_graspable_1 = (thumb_pos[:, 2] < handle_pos[:, 2]) & (index_pos[:, 2] > handle_pos[:, 2]) & (middle_pos[:, 2] > handle_pos[:, 2])
    is_graspable_2 = (thumb_pos[:, 2] > handle_pos[:, 2]) & (index_pos[:, 2] < handle_pos[:, 2]) & (middle_pos[:, 2] < handle_pos[:, 2])
    # bonus if left finger is above the drawer handle and right below
    return is_graspable_1 + is_graspable_2


def target_qpos_reward(env: ManagerBasedRLEnv, target_qpos, asset_cfg: SceneEntityCfg, target_frame: str = 'cabinet_frame'):
    arm_joint_pos = env.scene[asset_cfg.name].data.joint_pos[:, asset_cfg.joint_ids]
    if isinstance(target_qpos, torch.Tensor):
        target_qpos = target_qpos.clone().detach().to(device=arm_joint_pos.device)
    else:
        target_qpos = torch.tensor(target_qpos, device=arm_joint_pos.device)
    is_graspable = align_grasp_around_handle(env, target_frame=target_frame).float()
    return -(1 - is_graspable) * torch.norm(torch.abs(target_qpos - arm_joint_pos) / torch.abs(target_qpos), dim=-1)


def approach_gripper_handle(env: ManagerBasedRLEnv, offset: float = 0.04, target_frame: str = 'cabinet_frame') -> torch.Tensor:
    ...
    thumb_dist = torch.abs(thumb_pos[:, 2] - handle_pos[:, 2])
    index_dist = torch.abs(index_pos[:, 2] - handle_pos[:, 2])
    middle_dist = torch.abs(middle_pos[:, 2] - handle_pos[:, 2])

    # Check if hand is in a graspable pose
    is_graspable = align_grasp_around_handle(env, target_frame=target_frame).float()

    return (is_graspable) * ((offset - thumb_dist) + (offset - index_dist) + (offset - middle_dist))


def grasp_handle(
    env: ManagerBasedRLEnv, threshold: float, open_joint_pos: float, asset_cfg: SceneEntityCfg, target_frame: str
) -> torch.Tensor:
    ...
    is_graspable = align_grasp_around_handle(env, target_frame=target_frame).float()
    return (is_close + is_graspable) * torch.sum(torch.abs(open_joint_pos - gripper_joint_pos), dim=-1)


def open_drawer_bonus(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, target_frame: str) -> torch.Tensor:
    ...
    return (is_graspable + 0.1) * drawer_pos


def multi_stage_open_drawer(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, target_frame: str) -> torch.Tensor:
